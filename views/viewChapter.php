            ...
            <?php foreach ($comments as $comment) : ?>
                ...
                if ($comment->checkComment() == 1 or $comment->checkComment() == 2) {
                    ...
                    if (!empty($_SESSION) and $comment->checkComment() == 1 and $_SESSION['id'] > 1 and $_SESSION['pseudo'] !== $comment->author()) {
                        echo "<p><a class=\"text-danger mode-signal\" href=\"\" data-toggle=\"modal\" data-id=\"" . $chapter->id() . "\" data-idpost=\"" . $comment->id() . "\" data-target=\"#modalSignalComment\">Signaler le commentaire</a></p>";
                    } else if ($comment->checkComment() == 2 AND !empty($_SESSION)) {
                        echo "<img class=\"img-warning\" src=\"public/images/Warning.png\"><p class=\"text-danger mode-signal\" >Ce commentaire a été signaler !</p>";
                    }
                    echo "</div>";
                    echo "</div>";
                }
